...
void reconnect() { 
  ...
  while (!client.connected()) {
    ...
    if (client.connect("SPC1")) {
      ...
      client.subscribe("Set_Speed1M");
      client.subscribe("Steering_DP 1");

      client.subscribe("Set_Speed4M");
...
// Mode Central
  ...
  client.publish("steering1", dtostrf(degree1,6,0,degree1_send));
  client.publish("steering4", dtostrf(degree2,6,0,degree2_send));
  client.publish("mesin1", dtostrf(rpm_filtered,6,0,rpm_send));
  client.publish("flow1", dtostrf(delta_orifice,6,0,delta_orifice_send));
  client.publish("rpm1", dtostrf(rpm_filtered,6,0,rpm_send));
  client.publish("rpm4", dtostrf(rpm2_filtered,6,0,rpm2_send));
  client.publish("flow_lpm", dtostrf(float_lpm_filtered,6,0,flow_lpm_send));
  client.publish("flow_lpm2", dtostrf(float_lpm_filtered2,6,0,flow_lpm2_send));
...
if(rpm_time > 62){
  ...
  flow_lpm = flow_tick * 1.707692308;
  float_lpm_filtered = (0.7 * flow_lpm) + (0.3 * float_lpm_filtered);

  flow_lpm2 = flow_tick * 1.707692308;
  float_lpm_filtered2 = (0.7 * flow_lpm) + (0.3 * float_lpm_filtered);
 
  if (float_lpm_filtered < 3){
    float_lpm_filtered = 0;
  }

  if (float_lpm_filtered2 < 3){
    float_lpm_filtered2 = 0;
  }
...
time_loop_prev = millis();
  ...
  if(flow_state2 != flow_state_prev2){
    flow_tick2++;
  }
