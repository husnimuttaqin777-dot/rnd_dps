			...
			Button{
				...
				MapQuickItem {
					...
					sourceItem: Rectangle {
						...
						Image {
							anchors.fill: parent
							source: "cross_green.png" // Ignore warnings from this
				...
				/*
				 MapPolyline {
					...
					path: [
						...
						{ latitude: latitude_position_value.text, longitude: longitude_position_value.text },
						{ latitude: latitude_destination.text, longitude: longitude_destination.text}
					]
				 }
				 */
...
Window {
		...
		Button{
			...
			onClicked:{
				if (ship_type.checked == true){
					thruster_type.checked = false
					control_type.checked = false
					rpl.checked = false

					ship_type_page.visible = true
					thruster_type_page.visible = false
		...
		Button{
			...
			Rectangle{
				width : parent.width
				height : parent.height
				color : thruster_type.checked ? "blue" : "gray"
			}

			onClicked:{
			if (thruster_type.checked == true){
					...
					thruster_type_page.visible = true
					control_type_page.visible = false
					navigation_page.visible = false
				}
			}
		...
		Rectangle{
			...
			ComboBox {
				x : 135
				y : 40
				model: ["Azimuth Thruster", "Bow Thruster", "Rudder Thruster"]
				height : 25
			}
			...
			ComboBox {
				x : 135
				y : 70
				model: ["Stepper", "Selenoid Valve", "DC Motor"]
				height : 25
			}
			...
			CheckBox {
				...
				Text {
					x : 25
					text: qsTr("Invert")
					color: "#00ff00" // Mengatur warna teks menjadi hijau
					font.pixelSize : 14
				}
			}
	...
	Timer{
		...
		radar_gauge.height = constrain(mapValue(backend.position_error(),0,1,0,75), 0, 75)
