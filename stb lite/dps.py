...
#sys.path = ["./lib"]
# BUANG semua path Thonny
#sys.path = [p for p in sys.path if "Thonny" not in p]
...
def _get_color(value):
    """Pre-compute polygon fill color in Python — avoids QML ternary chain on every repaint."""
    if   value <  0:  return "#a80000"
    elif value <  4:  return "#df4000"
    elif value <  8:  return "#f37700"
    elif value < 12:  return "#f8ab00"
    elif value < 16:  return "#f8d800"
    elif value < 20:  return "#f2f200"
    elif value < 24:  return "#cef400"
    elif value < 28:  return "#87e602"
    elif value < 32:  return "#21d824"
    elif value < 36:  return "#00c846"
    elif value < 40:  return "#00b46b"
    elif value < 44:  return "#009d8d"
    elif value < 48:  return "#00b8d3"
    elif value < 60:  return "#00daf8"
    else:             return "white"



def _rdp_simplify(points, epsilon=0.00003):
    """
    Ramer-Douglas-Peucker polygon simplification.
    Reduces vertex count — biggest GPU/CPU win on Armbian ARM hardware.
    Tune epsilon: larger = fewer points = faster rendering.
    """
    if len(points) < 3:
        return points

    def _pt_line_dist(p, s, e):
        if s == e:
            return math.hypot(p[0] - s[0], p[1] - s[1])
        dx, dy = e[0] - s[0], e[1] - s[1]
        mag2 = dx*dx + dy*dy
        u = max(0.0, min(1.0, ((p[0]-s[0])*dx + (p[1]-s[1])*dy) / mag2))
        return math.hypot(p[0] - (s[0] + u*dx), p[1] - (s[1] + u*dy))
    ...
    simplified = _rdp(tuples)
    return [QGeoCoordinate(lat, lon) for lat, lon in simplified]


def load_polygons(label_min_area=0.000001):
    """
    Load isobath polygons with:
    - RDP vertex simplification (fewer GPU draw calls)
    - Pre-computed color string (no QML ternary chain per frame)
    - showLabel flag (skip MapQuickItem for tiny polygons)
    """
    ...
    for comp in component_polygons.values():
        ...
        # Simplify vertices — major performance gain
        coords = _rdp_simplify(coords, epsilon=0.00003)
        if len(coords) < 3:
            continue
        ...
        # Bounding-box area — skip label on tiny polygons to reduce MapQuickItem count
        ...
        polygons.append({
            ...
            "type":      comp["type"],
            "value":     val,
            "color":     _get_color(val),         # polygon fill — no QML ternary chain
            "center":    {"latitude": center_lat, "longitude": center_lon},
            "showLabel": True,  # show label on all polygons (tune label_min_area to filter tiny ones)
        })

    print(f"[load_polygons] {len(polygons)} polygons loaded, "
